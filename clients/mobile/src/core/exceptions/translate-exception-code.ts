...
const englishExceptionTranslations = {
	...
	sender_required_to_delete_message: "Only the sender can delete this message.",
...
const frenchExceptionTranslations = {
	...
	collection_name_already_exists: "Une collection portant ce nom existe déjà.",
	...
	parent_message_not_found: "Message parent introuvable dans cette discussion.",
...
function translateExceptionCode(code: unknown): string {
	...
	if (typeof code === "string") {
		const normalizedCode = code.replace(/^exception_/, "") as ExceptionCode;
		if (normalizedCode in translations) {
			return translations[normalizedCode];
		}
	}
