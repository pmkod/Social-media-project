...
export function Sidebar() {
	return (
		<aside className="hidden md:flex flex-col justify-between w-64 lg:w-72 p-4 h-screen sticky top-0 border-r border-slate-200/80 dark:border-slate-800 shrink-0">
					...
					{NAV_ITEMS.map((item) => {
						...
						return (
							<Link
								key={item.to}
								to={item.to}
								className="flex items-center gap-3.5 px-4 py-3 rounded-2xl text-slate-700 dark:text-slate-300 font-medium text-sm hover:bg-slate-200/60 dark:hover:bg-slate-800/60 hover:text-slate-900 dark:hover:text-slate-100 transition-colors"
								activeProps={{
									className:
										"bg-sky-500/10 text-sky-600 dark:text-sky-400 font-semibold hover:bg-sky-500/15",
								}}
							>
