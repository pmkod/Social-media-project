...
import {
	...
	useUserConnections,
} from "./use-user-connections.ts";

type UserConnectionsModalProps = {
	userId: string;
	username: string;
	type: UserConnectionType;
};

const UserConnectionsModal = create(
	({ userId, username, type }: UserConnectionsModalProps) => {
		const modal = useModal();
		const scrollContainerRef = useRef<HTMLDivElement>(null);
		const observerTargetRef = useRef<HTMLDivElement>(null);
		const query = useUserConnections(userId, type);
