import { RiAddLine, RiFlashlightFill } from "@remixicon/react";
...
function SparksPage() {
	...
	const query = useSearchPosts({ query: q, type: "SPARK" });
		...
		<main className="min-w-0 flex-1 bg-muted/20 px-3 pb-20 md:px-6 md:pb-4">
			<header className="mx-auto flex max-w-3xl items-center justify-between gap-3 py-5">
				<div>
					<h1 className="flex items-center gap-2 text-2xl font-bold">
						<RiFlashlightFill className="size-7 text-primary" />
						Sparks
					</h1>
			...
			{q ? (
				<div className="mx-auto mb-4 flex max-w-3xl items-center justify-between gap-2 text-sm">
					<p>Results for “{q}”</p>
					<Link to="/sparks" search={{}} className="text-primary">
						All Sparks
					</Link>
				</div>
			) : null}
			<section
				ref={feedRef}
				key={q}
				aria-label="Sparks feed. Scroll to see the next video."
			...
			>
				{query.isPending ? (
					...
					<EmptyBlock
						title={q ? "No Sparks found" : "Start something bright"}
						description={
							q
								? "Try searching for another moment or topic."
								: "Share the first Spark: a video of up to 90 seconds."
						}
					/>
				) : (
					posts.map((post) => (
						<div key={post.id} className="snap-start pb-6">
							<SparkItem post={post} paused={creating} />
						</div>
					))
				)}
				{query.hasNextPage ? (
					...
					<p className="py-6 text-center text-sm text-muted-foreground">
						You're all caught up. Come back for more Sparks.
					</p>
				) : null}
			</section>
			<Dialog
